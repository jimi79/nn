...
def datas():
	y = []
	count = 1000 # number of examples i generate
	maxv = 10 # max value for a, b and c (let's start slow)
...
def train(): 
	...
	s=d.train([16,16,16], 0.001, 0.01, 100000, 1) # the layout is here, 2 hidden layers of 64 neuros. last layer is to go from 64 to the expected 9 bits
	return d, s
...
def test(d, s, a, b, c):
	return n.binary_to_int( d.FP(None, s, np.array([convertXtoI([a,b,c], 4)])) >= 0.5)
